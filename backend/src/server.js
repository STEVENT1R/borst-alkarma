...
async function startServer() {
  try {
    ...
    // إضافة الأعمدة المفقودة وتحسين القيود (للتحديث)
    await db.query(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS title VARCHAR(100) NOT NULL DEFAULT ''`).catch(() => {});
    await db.query(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS supervisor_id INTEGER REFERENCES users(id)`).catch(() => {});
    await db.query(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS worker_id INTEGER REFERENCES users(id)`).catch(() => {});
    await db.query(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS receiver_name VARCHAR(100)`).catch(() => {});
    await db.query(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS price DECIMAL(10,2) DEFAULT 0`).catch(() => {});
    await db.query(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS reminder_time TIMESTAMPTZ`).catch(() => {});
    await db.query(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS sale_type VARCHAR(20) DEFAULT 'retail'`).catch(() => {});
    await db.query(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS notes TEXT`).catch(() => {});
    await db.query(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ`).catch(() => {});
    // تحديث CHECK constraint للحالة
    await db.query(`ALTER TABLE tasks DROP CONSTRAINT IF EXISTS tasks_status_check`).catch(() => {});
    await db.query(`ALTER TABLE tasks ALTER COLUMN status SET DEFAULT 'pending'`).catch(() => {});
    await db.query(`ALTER TABLE tasks ALTER COLUMN status TYPE VARCHAR(30)`).catch(() => {});
    // تغيير description ليكون nullable لو مش null
    await db.query(`ALTER TABLE tasks ALTER COLUMN description DROP NOT NULL`).catch(() => {});

    // 9. Salaries table - الرواتب
    await db.query(`
    ...
    // Salary payments table - سجل إيداع المرتبات
    await db.query(`
    ...
    // 12. Receiver transactions - سجل تعاملات المستلمين
    await db.query(`
    ...
    // إضافة task_id إلى inventory_transactions لو مش موجود
    await db.query(`ALTER TABLE inventory_transactions ADD COLUMN IF NOT EXISTS task_id INTEGER REFERENCES tasks(id)`).catch(() => {});

    // إعدادات افتراضية
    await db.query(`INSERT INTO app_settings (key, value) VALUES ('cleanup_months', '6') ON CONFLICT (key) DO NOTHING`).catch(() => {});
