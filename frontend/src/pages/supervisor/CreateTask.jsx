      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow-sm border space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <select 
              value={receivers.some(r => r.name === form.receiver_name) ? form.receiver_name : ''} 
              ...
              {[...receivers]
                ...
                .map(r => (
                  <option key={r.id} value={r.name}>
                    {r.name} {r.balance > 0 ? `(مديون: ${r.balance} ج.م)` : ''}
                  </option>
                ))}
            </select>
          </div>
          ...
          {!receivers.some(r => r.name === form.receiver_name) && (
            <input 
              ...
              onChange={e => setForm({...form, receiver_name: e.target.value})} 
              className="w-full p-3 border border-gray-200 rounded-xl bg-gray-50 mt-2" 
            ...
            <div className="mt-2">
              <details className="text-sm">
                <summary className="text-blue-600 cursor-pointer font-medium">
                  كل المستلمين ({receivers.length})
                  {form.receiver_name && receivers.some(r => r.name === form.receiver_name) && (
                    <span className="text-green-600 mr-2">✓ {form.receiver_name}</span>
                  )}
                </summary>
                  ...
                  {[...receivers]
                    ...
                    .map(r => (
                      <button
                        key={r.id}
                        type="button"
                        onClick={() => setForm({...form, receiver_name: r.name})}
                        className={`text-right p-2 rounded-lg border text-xs transition-all ${
                          form.receiver_name === r.name
                            ? 'bg-blue-100 border-blue-300 text-blue-800'
                            : 'bg-white border-gray-200 text-gray-700 hover:border-blue-300'
                        }`}
                      >
                        <div className="font-bold">{r.name}</div>
                        <div className={`font-bold ${r.balance > 0 ? 'text-red-600' : 'text-green-600'}`}>
                          {r.balance > 0 ? `ديون: ${r.balance} ج.م` : 'مفيش ديون'}
                        </div>
                      </button>
                    ))}
                </div>
